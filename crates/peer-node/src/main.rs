...
async fn main() -> Result<()> {
    ...
    // ---------- Engine --------------------------------------------
    let callbacks = Callbacks { tx: transport.tx.clone()};
    let mut engine = InternalTableState::new(
        callbacks,
        TableId::from(opt.table_id().clone()),
        opt.seats,
        std::sync::Arc::new(signing_key.clone()),
    );

    // join myself (100 000 starting chips just for dev)
    engine.try_join(&peer_id, &opt.nick, Chips::new(100_000)).await?;
...
fn load_or_generate_key(path: &std::path::Path) -> Result<SigningKey> {
    ...
    if path.exists() {
        ...
        fs::File::create(path)?.write_all(bincode::serialize(&key)?.as_slice())?;
        Ok(key)
    }
}
...
impl EngineCallbacks for Callbacks {
    ...
    fn credit_chips(&mut self, _dest: PeerId, _chips: Chips) -> Result<(), anyhow::Error> {
        Ok(())
    }
}
