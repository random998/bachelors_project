...
#[must_use] pub fn start(
    ...
    handle.spawn(async move {
        // --- libp2p transport ------------------------------------
        let transport =
            crate::swarm_task::new(&table, keypair.clone(), seed_addr,);

        // --- poker engine ----------------------------------------
        #[derive(Clone,)]
        struct Cb {
            tx_gossip: P2pTx,
            tx_ui:     mpsc::Sender<SignedMessage,>,
        }
        ...
        eng.try_join(&signing.peer_id(), &nickname, Chips::new(100_000,),)
            .await
            .unwrap();
        ...
        loop {
            ...
            while let Ok(m,) = rx_ui.try_recv() {
                eng.handle_message(m,).await;
            }
            // 2) gossip → engine
            while let Ok(m,) = eng.connection.rx.receiver.try_recv() {
                eng.handle_message(m,).await;
            }
            // 3) timeouts
            eng.tick().await;
