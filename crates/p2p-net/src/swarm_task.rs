...
use libp2p::{identify, noise, yamux, SwarmBuilder, Transport};
use libp2p::tcp;
...
pub fn new(table_id: &TableId, keypair: poker_core::crypto::KeyPair,) -> P2pTransport {
    // identity ------------------------------------------------------
    ...
    let transport =
        tcp::tokio::Transport::new(tcp::Config::default().nodelay(true,),)
            ...
            .authenticate(noise::Config::new(&libp2p_identity::Keypair::from(keypair.0.clone(),)).unwrap(),)
    ...
    let gossip = gossipsub::Behaviour::new(
        gossipsub::MessageAuthenticity::Signed(libp2p_identity::Keypair::from(keypair.0.clone()),),
    ...
    let mut swarm = SwarmBuilder::with_existing_identity(libp2p_identity::Keypair::from(keypair.0.clone()),)
        .with_tokio() // runtime
        .with_tcp(
            // transport
    ...
    swarm.listen_on("/ip4/0.0.0.0/tcp/0".parse().unwrap()).expect("listen on /ip4/0.0.0.0/tcp/0 failed");
    ...
    if swarm.listeners().count() > 0 {
        info!("{} listening on {}", peer_id, swarm.listeners().next().expect("no listeners for swarm"));
    } else {
