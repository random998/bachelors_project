...
//! A fully in-memory integration test for peers joining a table in various scenarios.
//! All tests use in-memory channels to simulate P2P communication without a real network.
//! Messages are sent directly between peers using pairwise channels, leveraging the
//! `SignedMessage` with embedded `PeerId` to identify senders.
...
// Simulates network message passing by processing ticks and handling received messages.
async fn pump_messages(alice: &mut Projection, bob: &mut Projection) {
    loop {
        let mut progressed = false;
        alice.tick().await;
        if let Ok(msg) = alice.try_recv() {
            info!("{} received message: {:?}", alice.peer_id().to_string(), msg.message().label());
            alice.handle_network_msg(msg).await;
            progressed = true;
        }
        bob.tick().await;
        if let Ok(msg) = bob.try_recv() {
            info!("{} received message: {:?}", bob.peer_id().to_string(), msg.message().label());
...
/// - All peers end with consistent state: same players list (sorted or in join order), hash_head, chain length.
...
pub async fn three_peers_join_success() -> Result<()> {
    ...
    assert_eq!(alice.hash_head(), bob.hash_head());
    assert_eq!(alice.hash_head(), charlie.hash_head());
    assert!(alice.players().iter().all(|p| p.seat_idx.is_some()), "Seats assigned");

    Ok(())
}

/// Test rejection when table is full: Alice and Bob join, Charlie tries to join but table size 2.
///
/// This test verifies:
...
async fn reject_invalid_sync_resp() -> Result<()> {
    ...
    // Empty Alice's outbound queue to simulate message sent but not received by Bob
    ...
    if let Ok(_) = alice.try_recv() {  // Consume SyncReq
    ...
    assert!(bob.players().iter().find(|p| p.peer_id == bob.peer_id()).is_none()); // bob should not have joined his own state.
...
/// Test concurrent joins: Bob and Charlie send SyncReq at the same time, ensure chain consistency.
///
/// This test verifies:
...
///
/// This test verifies:
/// - Seats are assigned deterministically (e.g., based on join order or PeerId).
...
/// Test non-seed peer trying to join without seed present: Should fail or hang, but in test, no response.
///
/// This test verifies:
/// - Non-seed peer sends SyncReq, but without seed, no SyncResp, so doesn't join.
...
async fn non_seed_join_no_seed() -> Result<()> {
    ...
    let mut bob = Projection::new(
        ...
        6,
        kp_b.clone(),
        mock_transport(tokio::sync::mpsc::channel(64).0, tokio::sync::mpsc::channel(64).1),  // Dummy transport
        |_| {},
        false,
    );
...
/// Test chain replay with multiple joins: Bob joins after Alice and Charlie, replays full chain.
///
/// This test verifies:
