...
/// Test successful join of two peers: seed (Alice) joins directly, non-seed (Bob) sends SyncReq -> Alice processes, appends JoinTableReq, sends SyncResp and ProtocolEntry
...
async fn two_peers_join_success() -> Result<()> {
    ...
    assert_eq!(bob.players().len(), 1,  "Bob should see Alice after processing her join");
    assert_eq!(alice.hash_head(), bob.hash_head(), "Hashes match after Alice's join");
    ...
    // pump to propagate: Bob sends SyncReq -> Alice processes, appends JoinTableReq, sends SyncResp and ProtocolEntry.
    pump_messages(&mut alice, &mut bob).await;
