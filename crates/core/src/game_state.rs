...
impl Projection {
    ...
    pub fn get_player(&self, id: &PeerId) -> Option<PlayerPrivate> {
        self.contract.get_players().iter().find(|p| p.peer_id == *id).cloned()
    }
...
impl Projection {
    ...
    fn update_players(&mut self, updates: &[PlayerUpdate]) {
        for update in updates {
            let id = self.peer_id();
            if let Some(player) = self.contract.get_players_mut().find(|p| p.peer_id == update.player_id) {
...
impl Projection {
    ...
    fn enter_start_game(&mut self, timeout_s: u64, max_retries: u32) {
        ...
        self.start_game_timeout = Some(
            Instant::now() + Duration::from_secs(timeout_s * u64::from(max_retries)),
        );
    }
    ...
    fn enter_start_hand(&mut self) {
        ...
        for &id in &active_ids {
            ...
            if let Some(player) = self.contract.get_players_mut().find(|p| p.peer_id == id) {
    ...
    fn enter_end_hand(&mut self) {
        ...
        if self.count_with_chips() < 2 {
            ...
            // start of a new hand.
            let players = self.get_players().clone();
    ...
    fn handle_multi_player_payoff(&mut self, payoffs: &mut Vec<HandPayoff>) {
        ...
        for (idx, &(id, v, ref bh)) in hands.iter().take(winners_count).enumerate() {
            ...
            if let Some(payoff) = payoffs.iter_mut().find(|po| po.player_id == id) {
        ...
        for player in self.get_players() {
            if player.is_active {
                ...
                match player.action {
                    PlayerAction::None
                    | PlayerAction::SmallBlind
                    | PlayerAction::BigBlind if player.chips > Chips::ZERO => {
                        return false;
                    }
                    _ => {}
                }
    ...
    fn request_action(&mut self) {
        if let Some(id) = self.active_player_id {
            let player = self.contract.get_players_mut().find(|p| p.peer_id == id).expect("err");
