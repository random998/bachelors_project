...
impl<'de> Deserialize<'de> for Hash {
        ...
        D: Deserializer<'de>,
    {
        ...
        let hash = blake3::Hash::from_slice(bytes).expect("error deserializing hash");
        Ok(Hash(hash))
    }
}
...
pub enum WireMsg {
    ...
    PlayerJoinedConf {
        table: TableId, player_id: PeerId, seat_idx: u8, chips: Chips,
    },
    LeaveTable { player_id: PeerId },

    /* ── game bootstrap & shuffle ───────────────────────── */
    StartGameNotify {
        table: TableId, game_id: GameId, seat_order: Vec<PeerId>,
        sb: Chips, bb: Chips,
    },
    ShuffleCommit {
    ...
    /* ── betting ─────────────────────────────────────────── */
    ActionRequest {
        table: TableId, game_id: GameId, player_id: PeerId,
        min_raise: Chips, big_blind: Chips, allowed: Vec<PlayerAction>,
    },
    PlayerAction {
        table: TableId, game_id: GameId, player_id: PeerId,
        action: PlayerAction,
