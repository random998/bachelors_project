...
impl MockUi {
    pub fn new(keypair: KeyPair, nick: String, seed_addr: Option<Multiaddr>, num_seats: usize, table_id: TableId) -> MockUi {
    ...
    pub fn default(seed_addr: Option<Multiaddr>, table_id: TableId, nick: String) -> MockUi {
        let kp = KeyPair::generate();

        Self::new(kp, nick, seed_addr, 3, table_id)
    }

    pub async fn send_to_engine(&mut self, ui_cmd: UIEvent) -> Result<(), anyhow::Error> {
        let err = self.ui_handle.cmd_tx.send(ui_cmd).await.map_err(|e| e.into());
        // update to pull the latest gamestate after sending msg to engine.
        self.poll_game_state().await;
        err
    }

    pub fn try_recv_from_engine(&mut self) -> Result<EngineEvent, anyhow::Error> {
        self.ui_handle.msg_rx.try_recv().map_err(|e| e.into())
    }
    ...
    pub async fn wait_for_listen_addr(&mut self) {
        ...
        loop {
            ...
            attempts += 1;
            assert!(attempts <= max_attempts, "Timeout waiting for listen addr");
        }
    }
    ...
    pub async fn poll_game_state(&mut self, ) {
        ...
        loop {
            ...
            tries += 1;
            if tries >= max_tries {
                info!("{} did not update gamestate", self.last_gamestate.nickname.clone());
                break;
            }
        }
    }
